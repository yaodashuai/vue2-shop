<template>
  <div class="pay">
    ...
    <div class="pay-list" v-if="order.goodsList">
      ...
      <!-- 买家留言 -->
      <div class="buytips">
        <textarea placeholder="选填: 买家留言(50字以内)" v-model="remark" name="" id="" cols="30" rows="10"></textarea>
      </div>
    </div>
  ...
  methods: {
    ...
    async getData () {
      ...
      if (this.mode === 'buyNow') {
        ...
        obj.goodsSkuId = this.goodsSkuId
        obj.goodsNum = this.goodsNum
      } else {
        obj.cartIds = this.cartIds
      }

      const { data: { personal, order } } = await checkOrder(this.mode, obj)
