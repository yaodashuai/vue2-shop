  ...
  <div class="cart">
   ...
   <div v-if="isLogin && cartList.length > 0">
    ...
    <div class="footer-fixed">
      <div  class="all-check">
        <van-checkbox @click="toggleAllCheck" :value="isAllCheck" icon-size="18"></van-checkbox>
        全选
      </div>
      ...
      <div class="all-total">
        ...
        <div v-if="!isEdit" class="goPay" :class="{ disabled: selCartList === 0 }" @click="goPay">结算({{ selCartCount }})</div>
        <div v-else class="delete" @click="delSelect">删除</div>
      </div>
    </div>
   </div>
...
export default {
    ...
    goPay () {
      if (this.selCartCount > 0) {
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.selCartList.map(i => i.id).join(',')
          }
        })
      }
    }
  ...
  computed: {
    ...
    ...mapGetters('cart', ['cartTotal', 'selCartList', 'selCartCount', 'selCartPrice', 'isAllCheck']),
    isLogin () {
